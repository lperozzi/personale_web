...
The easiest way to apply the NN algorithm is by using the [Verde](https://www.fatiando.org/verde/latest/) package (Uieda L., 2018)^1. The dataset has been BlockReduced to a regular grid of 50m x 50m.
...
Kriging is a widly used spatial interpolator. Here we use the [GeostatPy][https://github.com/GeostatsGuy/GeostatsPy) package (Pyrcz et al., 2021)<sup>2<\sup>, which is the python version of translated to Python from the original FORTRAN GSLIB: Geostatistical Library methods (Deutsch and Journel, 1997).
